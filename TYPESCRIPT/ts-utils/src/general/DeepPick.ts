...
export type DeepPick<T, Filter extends DeepModify<T>> = T extends BuiltIn
	...
	? {
			...
			[K in keyof T as K extends keyof Filter ? K : never]: Filter[K & keyof Filter] extends true ? T[K] : K extends keyof Filter ? (Filter[K] extends DeepModify<T[K]> ? DeepPick<T[K], Filter[K]> : never) : never;
	  }
	: never;

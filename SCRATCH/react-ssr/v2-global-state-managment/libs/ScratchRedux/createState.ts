...
export class CreateState<S extends any = any>
  implements CreateStateInterface<S> {
  private state: S;
  private prevState!: S;
  ...
  private callCompare = () => {
    ...
    const updated = this.compareFunc(this.prevState, this.state);
    if (updated) {
      this.callListeners();
    }
  };
  ...
  public setState = <K extends keyof S>(
    ...
    if (typeof state === "function") {
      this.state = (state as (prevState: Readonly<S>) => S | Pick<S, K>)(
        this.prevState
      ) as S;
    } else {
      this.state = state as S;
...
export default function createState<S>(
  initialState: S,

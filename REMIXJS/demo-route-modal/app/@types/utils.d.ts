declare type ExtractRouteParams<T extends string> = T extends `${infer _Pathname}:${infer Param}/${infer Rest}`
  ? {
      [k in Param | keyof ExtractRouteParams<Rest>]: string;
    }
  : T extends `${infer _Pathname}:${infer Param}`
  ? {
      [k in Param]: string;
    }
  : {};
...
declare type Push<T extends any[], E> = ((head: E, ...args: T) => any) extends (
  head: infer Element,
  ...args: infer Array
) => any
  ? [...Array, Element]
...
declare type AppendArgument<Fn, ArgType> = Fn extends (...args: infer T) => infer R
  ? (...args: [...T, ArgType]) => R
  : never;

declare type FunctionDetail<F extends Function> = F extends (...args: infer Params) => infer ReturnType
  ? [Params, ReturnType]
  : never;
...
declare type StringToUnion<T extends string> = T extends `${infer Character}${infer Rest}`
